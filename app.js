...
form.addEventListener("submit",(e)=>{
    ...
    if(isNaN(value) || (value <= 0 || value > 9)){
         ...
         let randTxt = text[randNum];

        //  console.log(randTxt);
         article.innerHTML = `<p class="active">${randTxt}</p>`; 
    }else {
             
        const items = text.slice(0,value);
        items.reverse()
        // console.log(newArticles);

        const newArticles = items.map(article =>{
            return `<p class="active">${article}</p>`
        }).join('')
        console.log(newArticles);
        
        article.innerHTML = newArticles; 
